...
description: "Apply the user's engineering design style to implementation, debugging, architecture, refactoring, review, tooling, games, and UI. Use when software work needs clear ownership, one source of truth, logic/presentation separation, configurable behavior, code conventions, reuse of existing APIs, and verifiable workflows."
---
...
需要更多来源材料时，读取 `references/source-notes.md`。任务涉及游戏引擎组件、UI、客户端或其他表现入口时，完整读取 `references/logic-presentation-boundaries.md`。实施、重构或代码评审还必须读取 `references/code-quality-standards.md`；项目已有格式化、静态检查、命名或目录约定时，以项目规则为准。

## 工作流程

1. 先理解需求：确认用户要解决的问题、想要的效果、不做什么、有什么限制。
2. 先找现有实现：搜索已有逻辑模块、配置、事件、工具、引擎组件、UI、服务、API、共享契约和调用范式，同时搜索同功能的 V1/V2、Old/New、Legacy、兼容层和历史入口。新增表现入口时，按项目实际形态检查业务拥有者和同类表现；不要只复用当前组件附近的旧代码，也不要为单进程项目凭空制造远程后端。
...
“复用现有实现”优先指复用稳定的逻辑拥有者、公开入口和成熟工具，不是机械复制表现层附近的历史旁路。项目文档和旧代码是重要证据，但如果它们与真实调用链、其他表现入口、唯一真源或用户明确风格冲突，应先把矛盾讲清楚，再决定迁移或兼容方式。
...
- 现有逻辑模块、Service、API、共享契约和其他表现入口中有哪些能力可以复用；当前项目适合直接模块调用、事件、共享库还是远程接口。
...
V2 进入正式调用链时，不能只新增 V2 再保留 V1。逐项确认 V1 的状态，并在同一次改动完成收尾：

1. **旧 API、存档、序列化资源或外部调用仍会到达 V1**：保留最小兼容层，并用语言或框架支持的过时标记（如 `Obsolete`、`@deprecated`）或紧邻注释写明“已过时”、替代入口、触发版本/条件、删除条件和最晚删除版本。兼容层只能转换或转发到 V2，不能保留第二套业务规则。
2. **不存在运行时兼容需要**：先确认 V1 所在文件或目录是否实际纳入版本控制，并且历史可以通过 Git、SVN 或项目实际使用的版本控制软件读取。已纳入且可追溯时，直接删除 V1；历史版本由版本控制恢复，不创建 `archive/` 副本。仅有仓库目录但目标文件被忽略、未跟踪、位于忽略目录或不在仓库范围内，不算已纳入版本控制。
3. **不存在运行时兼容需要，且 V1 文件没有可用的版本控制历史**：只在 V1 已确认无 bug、可正常运行、且作为完整旧版本具有保留价值时归档。把原文件移动到项目根目录 `archive/`，按相对项目根目录的路径完整镜像，不改变原有层级。例如 `Assets/Test.txt` 必须归档到 `archive/Assets/Test.txt`；文件名和扩展名保持不变，不复制第二份到原目录。`archive/` 必须排除编译、资源打包、代码生成、自动发现和正式入口索引，并在归档说明中写明替代版本、归档原因和原路径。
4. **V1 有 bug、无法确认正常运行、只是临时代码、重复逻辑或没有保留价值**：直接删除，不归档。

V1 被 V2 替代后，同一次改动检查并删除或迁移 V1 的调用点、注册、导出、配置键、资源引用、文档入口和只服务 V1 的测试。归档不是复制一套可运行实现；过时标记不是把整段旧代码注释掉后继续留在活跃源码目录。不得留下没有去向的 `Old`、`New2`、`Backup`、`Temp`、`Legacy` 文件、永久关闭的 feature flag 或“以后可能会用”的死逻辑。

## 代码质量基线

架构正确不等于代码合格。设计原则处理职责、数据归属和协作关系；代码规范处理单个文件、类型、函数和改动本身是否易读、可维护、可验证。两者都要检查，不能用“设计矛盾”掩盖命名、控制流、错误处理或死代码问题。

编码、重构或审查时：

1. 先找项目实际规则：格式化器、linter、类型检查、测试、`.editorconfig`、语言约定和同目录范式优先于本 Skill 的通用建议。
2. 缺少项目规则时，按 `references/code-quality-standards.md` 执行最低要求；不要凭个人偏好新增无关格式规则或批量格式化未改文件。
3. 每次改动只顺带整理与本次逻辑直接相关的可读性问题。需要较大整理时，拆成独立提交或先说明范围，不能把功能修复伪装成全文件重写。
4. 审查结论把“设计矛盾”和“代码规范问题”分开写：前者说明职责或真源错误；后者给出具体文件、行、规则和风险。两类问题可以同时存在。新版本替代旧版本时，必须额外给出旧入口的删除、归档或过时标记结果。
...
无论是单机游戏、联网应用还是本地工具，都使用同一套范式：稳定逻辑拥有业务事实、状态变化和规则；可替换的表现代码负责引擎生命周期、场景对象、UI、动画、特效、格式和交互。单机游戏可以是“纯逻辑模块 ↔ MonoBehaviour/Actor/Node/Component”，同进程直接方法调用或事件就是合理边界，不需要为了分层强造 HTTP、DTO、Service 或依赖注入。

具体问题具体分析：先确认什么需要稳定、什么需要替换、状态由谁解释，再选择模块调用、事件、共享库、Repository 或 API。已有逻辑入口时，表现层只做适配和可重建派生；没有拥有者时，新增合适的逻辑模块。禁止引擎组件或 UI 复制业务规则、另存正式状态或绕过既有拥有者。

场景落地、适度设计清单和验证门禁见 `references/logic-presentation-boundaries.md`。
...
检查所有表现入口是否复用同一逻辑拥有者。代码物理上分在不同目录、进程或语言里，不代表职责已经正确分离；反过来也不要过度架构：单机游戏中引擎组件直接调用逻辑模块完全合理，只要业务事实和可替换表现没有混在一起。
...
涉及不可逆数据覆盖时，按项目约定创建可恢复的数据备份或版本快照；备份不进入源码、资源、编译、自动扫描和正式文档索引。多步骤生成工具要让每一步的数据独立存在：当前步骤基于上一步生成自己的结果，不能静默覆盖上一步源数据。

### AI 协作方式

让 AI 先读现有实现和真实 API，再设计和修改。要求 AI 明确哪些地方复用、改哪些地方、不改哪些地方、数据怎么走、表现怎么刷新、怎么验证。

涉及新增引擎组件、UI、客户端、编辑器或其他表现入口时，开工前按复杂度输出边界与复用判断：业务事实拥有者、现有逻辑入口、表现层新增内容，以及是否真的需要新抽象。简单单进程功能可以简短说明；若表现层要新增正式状态、业务规则或数据读写，必须说明为什么不能归入逻辑模块。

当 AI 给方案时，先输出可核验的设计结构：模块职责、流程图、状态变化、模块关系、配置字段、事件列表或验证清单。不要只让 AI 直接堆代码。

当职责、数据归属、依赖或协作方式偏离这些设计习惯时，问题标签固定为“设计矛盾”；当命名、结构、控制流、错误处理、注释、日志、格式化或测试偏离代码规范时，问题标签固定为“代码规范问题”。不要用“设计矛盾”代替后者。
...
- 逻辑/表现边界：业务事实和规则归哪个模块；引擎组件、UI 或客户端只保留哪些生命周期适配、view model、渲染和交互状态。
- 边界与复用判断：当前应使用直接模块调用、事件、共享库、Repository 还是 API；是否存在不必要抽象或重复逻辑。
...
- 是否把数据状态和表现反馈混在一起了？
- 是否把“数据表现分离”误解成“每个表现入口各自实现一套业务逻辑”？
- 游戏引擎组件、UI 或客户端是否复用逻辑拥有者，只保存表现与生命周期状态？
- 是否根据项目实际选择直接调用、事件或 API，避免既复制逻辑又过度架构？
...
- 是否遵守项目格式化、命名、可见性、错误处理和测试约定；没有项目约定时，是否满足代码质量基线？
- 是否用与代码无关的架构术语掩盖了可直接定位的代码规范问题？
- V2 替代 V1 后，是否先确认了目标文件/目录的实际版本控制状态；V1 是否按“兼容层、已纳入版本控制直接删除、被忽略/未跟踪或无版本控制且是正常旧版本才归档、其余直接删除”收尾？
