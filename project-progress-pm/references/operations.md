...
按本次任务选择下方章节；不默认加载全表、完整聊天历史或全部归档。本文件的表格维护流程适用于腾讯文档是项目事实真源的情形；项目另有权威计划系统时，按其合同处理，不能直接套用下方表格状态规则。表头与状态为示例，必须核对实际字段。协作 Agent 仅在委派可用且获准时采用，否则由当前 Agent 完成同一动作。外部发送与写表分别遵从当前用户授权。

## PM 推进执行清单

每次执行项目推进时，执行者 先按“多 Agent 执行边界”分派可选协作 Agent。除非没有可用的多 agent 工具，否则下面需要读取腾讯文档、群消息、缓存和归档的步骤都由可选协作 Agent 完成，执行者 只审阅摘要和做最终决策。不要只看群消息就结束：

1. 执行者 读取用户当前会话和必要入口信息，确认本轮目标、项目上下文位置、授权边界、是否允许写表 / 发 QQ，以及需要启动哪些可选协作 Agent。
2. 缓存 / 归档可选协作 Agent 读取项目 PM 上下文和活跃缓存，确认当前仍在跟的事项、已归档事项、称呼 / QQ 映射、用户特别交代、工作时间 / 双休规则和在线表写回边界。
3. 腾讯文档巡检可选协作 Agent 读取腾讯文档当前页签，按表头动态识别列，重算当前有效事项、未闭环事项、状态分布、负责人负载、重点风险、待验收队列、已确认方案/实现队列，以及表格状态不一致处。
4. 腾讯文档巡检可选协作 Agent 必须先单独拉出 `0:重要紧急` 未闭环队列，按负责人和状态机分组：`A未开始/方案待确认` 要优先找负责人确认方案或缺口，`信息不足` 要找补资料，`C待验收` 才转 QA；不能因为有等待链或验收队列，就漏掉重要紧急的方案确认队列。
5. 缓存 / 归档可选协作 Agent 用稳定事项内容、负责人、状态、确认列、修正方案等重新定位缓存里的活跃事项；`Row N` 只能作为当时参考，不能直接当 ID。
6. 群消息巡检可选协作 Agent 读取最新群消息 / 私聊消息：被 @ 小助手的消息必须逐条处理并回复；普通群消息也要看，尤其是回复催办消息、负责人主动反馈、QA / 验收人结论、截图、排期、阻塞、用户指令和对项目状态有影响的闲聊式结论。
7. 如果群消息是回复小助手早前发言，群消息巡检可选协作 Agent 必须先读自己那条原消息和后续回复链，按“同一条对话继续往下接”处理；不要像第一次介入一样重新解释、重新报状态、或把后台缓存内容搬出来。
8. 执行者 合并群消息、在线表和缓存摘要：哪些已闭环，哪些需要验收，哪些只是收到，哪些仍缺信息，哪些需要换人跟，哪些已经不该再催，哪些是群聊中新产生的工作任务。
9. 对群聊中新产生的工作任务，执行者 指派腾讯文档巡检可选协作 Agent 去在线表查重：用问题标题、截图语义、功能名、负责人、最近群消息和相邻行判断是否已有记录；已有记录就形成写 `PM备注` / 状态 / 日期 / 截图证据的建议，不重复新增；没有记录且项目授权维护在线表时，新增或形成明确待写项，并保留来源 message_id。只要群消息带截图，就必须把截图链接、图片标记、文件 ID、本地下载路径或“截图存在但待补取”的 manifest 一并放进文档证据字段。
10. 执行者 先维护 PM 队列决策：本地缓存只记录定位线索、message_id、等待对象和下一步动作；工作项状态、负责人结论、闭环口径、验收口径等项目事实必须优先写回腾讯文档或按项目授权形成写回动作，不要只写进本地缓存。
11. 执行者 再决定是否说话：被 @ 小助手时必须短句回应；普通群消息不必每条插话，但如果影响项目状态、验收、排期、等待链、产生新任务或需要承接确认，就用简短自然的话回应。群里发言必须只承接群聊现场和群成员已知信息，不要把 Codex 侧用户调试语、内部缓存、表格统计、Row 清单或“我这边已记/我盯着”搬到群里。没有必要说话时，继续做在线表整理、队列拆分或下一工作窗口行动计划。
12. 能从回复得到明确结论时，执行者 优先安排写回 / 发送执行可选协作 Agent 维护在线表：通常写 `PM备注` / 跟进备注列，并在项目允许时整理 `状态`、`验收日期` 等字段；确认列要按项目规则谨慎处理。PM 不写 `修正方案批注`，该列留给人或方案审核者写；如果需要人补方案批注，只在 `PM备注` 记录“需补修正方案批注”的事实和依据。
13. 群消息巡检可选协作 Agent 检查当前时间和项目作息，再检查所有已发出的待回复问题：包括自己 @ 过的人、点名问过的排期 / 验收 / 结论、以及群里有人要求 PM 跟进后还没闭环的消息。先查 reply 链、近 24 小时普通群消息和缓存 message_id，确认是否已有间接回复；如果仍无结论且超过合理等待时间，该提醒就自然补一句，不要只处理最新消息。
14. 如果没有新回复，执行者 也要从可选协作 Agent 摘要里的活跃缓存和在线表风险中选下一件最值得推进的事项继续跟，而不是空跑结束。
15. 最后执行者 给用户一个简短汇总：本轮读到了什么、推进了什么、写表已写/待写什么、还在等谁、已经等了多久、下一步建议问谁。
...
```powershell
node "<read-tencent-docs-opendoc目录>\scripts\tencent-docs-mcp.mjs" get-sheet-info --url "<docs-url>"
```
...
```powershell
node "<read-tencent-docs-opendoc目录>\scripts\tencent-docs-mcp.mjs" get-range --url "<docs-url>" --range A1:AE80
```
...
项目明确使用 `ai-automation-workflow` 配套表结构时，按其状态机维护：
...
```powershell
node "<read-tencent-docs-opendoc目录>\scripts\tencent-docs-mcp.mjs" call-tool --name sheet.set_range_value --args "<json>"
```
