...
description: 按用户的编程设计风格巡检代码库或目标模块，并输出可视化设计矛盾报告。Use for architecture/style reviews, refactor opportunity discovery, frontend/backend reuse audits, multi-client duplicate business logic checks, data ownership or data/presentation separation reviews, and turning a selected conflict into an implementable solution.
---
...
巡检涉及客户端、前后端边界或多端复用时，完整读取 `../programming-design-style/references/frontend-backend-boundaries.md`。
...
- 前后端统一入口：新增 Web、桌面、托盘、移动端或编辑器时，应复用已有业务模块、统一后端、API 和共享契约；表现层可以替换，但不应复制业务读取、规则、权限、持久化或失败降级逻辑。
...
先搜索，再判断。使用快速代码搜索确认定义、调用点、数据所有权、配置来源、UI/表现路径、后端服务、API、共享契约、事件、编辑器工具和测试。

巡检目标是某个前端、托盘、移动端、编辑器或新表现入口时，不能把搜索范围限制在目标目录。必须沿一条真实能力链路追踪“前端 -> 适配器/客户端 -> 后端查询或命令 -> 业务事实拥有者 -> 持久化”，并检查 Web、桌面、移动端等其他客户端是否已经调用了可复用入口。
...
- 新前端重复文件/数据库访问、Repository、业务 Service、状态规则、权限或持久化，而项目已有统一后端或可扩展 API。
- 多个客户端实现同一业务能力，或同时保留 API 与本地旁路，形成可能分叉的事实源。
- presentation model / view model 包含会影响业务状态、权限、可执行动作、跨端一致性或持久化结果的规则，而不只是可重建的展示派生。
...
- **目标架构图**：展示建议收敛后的结构，明确哪些职责被移动、收拢、拆分或外置。涉及前端时，图中应明确表现端、客户端适配器、统一后端/领域模块、事实源和持久化边界。目标图可以是 Top Recommendation 对应的目标形态，而不是每个候选都画总图。
- **当前/目标对照摘要**：用 3-6 条短句说明从当前图到目标图，哪些“业务事实、状态、配置、表现、工具流程”的归属发生变化。
- **能力复用证据**：涉及新客户端或表现入口时，列出需求能力、现有业务拥有者、已有后端/API/共享契约、其他客户端调用方式，以及当前实现为什么复用或绕开了它。
...
- 表现/配置/工具影响：相关时说明 UI、动画、资源、配置或工具流程怎么变化；区分可自由替换的渲染/view model 与必须复用的业务查询、命令和事实源。
- 验证方式：用测试、编辑器检查、日志或手工流程如何证明改动正确。涉及多前端时，优先增加架构测试或依赖门禁，证明前端不直读后端管理的数据源、不复制领域规则，也不在后端失败时启用隐蔽旁路。
...
术语约束：偏离用户设计风格的问题固定称为“设计矛盾”。报告里的核心问题标签只使用“设计矛盾”。
